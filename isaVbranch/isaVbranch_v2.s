...
.globl isaVbranch
.p2align 8
.type isaVbranch,@function
.amdgpu_hsa_kernel isaVbranch
...
START_PROG:
	...
	// ------------------------------------------------------
	v_mov_b32			v[v_c],							0
	s_mov_b64			s[s_exec_bck:s_exec_bck+1],		exec						// backup exec
	v_and_b32			v[v_temp1],						0x01, v[v_tid_x]			// temp1 = tid_x & 0x01
	v_cmpx_eq_u32		vcc,							0x01, v[v_temp1]			// if(temp1 == 1) exec = 1; src0 should only use vcc
	
	v_mov_b32			v[v_c],							v[v_a]
	s_mov_b64			exec,							s[s_exec_bck:s_exec_bck+1]	// recover exec
...
.macro METADATA group_size, args_size
    ...
    { Version: [1, 0],
      Kernels :
        - { Name: isaVbranch, SymbolName: isaVbranch, Language: Assembler, LanguageVersion: [ 1, 2 ],
            Attrs:
