...
#define WAVE_NUM			(4)
#define GROUP_NUM			(60)
#define GROUP_SIZE 			(WAVE_SIZE * WAVE_NUM)
#define GLOBAL_SIZE			(GROUP_SIZE * GROUP_NUM)
#define STRIDE_LEN			(1024 + 64)
#define VECTOR_LEN			(GLOBAL_SIZE) // 每个 thread 64个DW时可以用一个wave充满一个L1
...
#ifdef OBJ_V3
	CreateAsmKernel("vmem_l1", "vmem_l1_v3.s");
#else
	CreateAsmKernel("vmem_l1", "vmem_l1_v2.s");
#endif
...
int main(int argc, char *argv[])
{
	...
	VectorLen = VECTOR_LEN * STRIDE_LEN;
	VectorLen = 1*1024*1024; // L2 size = 4MB = 1M DWORD
	InitHostMem();
	InitDeviceMem();
